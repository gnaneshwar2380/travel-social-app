...
const Profile = () => {
  ...
  const profilePictureUrl = profile.profile_pic
    ? `${getMediaUrl(profile.profile_pic)}?t=${timestamp}`
    : "/default-avatar.png";

  const coverPicStyle = profile.cover_pic
    ? {
        backgroundImage: `url(${getMediaUrl(profile.cover_pic)}?t=${timestamp})`,
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-20">
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-20">
        <div className="flex flex-col items-center bg-white p-6 rounded-lg shadow-lg relative">
          <img className="h-32 w-32 rounded-full border-4 border-white object-cover"
            src={profilePictureUrl} alt="Profile" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {posts.length > 0 ? (
                ...
                <div className="col-span-full flex flex-col items-center py-16 text-gray-400">
                  ...
                  <Link to="/create-trip" className="mt-4 bg-teal-500 text-white px-6 py-2 rounded-full text-sm font-semibold">Create Post</Link>
                </div>
              )}
            </div>
          )}
      ...
      {isModalOpen && (
        <EditProfileModal onClose={() => setIsModalOpen(false)} onUploadSuccess={fetchProfile} />
      )}
    </div>
  );
};
