        ...
        <div className="max-w-2xl mx-auto pb-24">
            <div className="sticky top-0 z-40 bg-white border-b px-4 py-3">
                <div className="flex items-center bg-gray-100 rounded-full px-4 py-2 gap-2">
                    ...
                    <input ref={inputRef} type="text" placeholder="Search people, trips, posts..."
                    ...
                    <div className="flex gap-2 mt-3 overflow-x-auto">
                        {tabs.map(tab => (
                            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                                className={`px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap ${activeTab === tab.key ? "bg-teal-500 text-white" : "bg-gray-100 text-gray-600"}`}>
                                {tab.label}
                            </button>
                        ))}
                    </div>
                    ...
                    <>
                                ...
                                <div className="space-y-2">
                                    {results.users.map((user) => (
                                        <div key={user.id} onClick={() => navigate(`/user/${user.username}`)}
                                            ...
                                            <img src={getMediaUrl(user.profile_pic)} alt={user.username}
                        ...
                        {(activeTab === "all" || activeTab === "trips") && results.trips.length > 0 && (
                            <div className="mb-6">
                                ...
                                <div className="space-y-4">
                                    {results.trips.map((trip) => <PostCard key={trip.id} post={trip} />)}
                                </div>
                            </div>
                        )}
                        ...
                        {(activeTab === "all" || activeTab === "posts") && results.posts.length > 0 && (
                            <div className="mb-6">
                                ...
                                <div className="space-y-4">
                                    {results.posts.map((post) => <PostCard key={`${post.post_type}-${post.id}`} post={post} />)}
                                </div>
                            </div>
                        )}
                    </>
