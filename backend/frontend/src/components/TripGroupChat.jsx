...
export default function TripGroupChat() {
    ...
    const sendMessage = async (e) => {
        ...
        try {
            const res = await api.post(`/groups/${groupId}/chat/`, { content: newMessage });
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-50 pb-16">
            <div className="bg-white border-b px-4 py-3 flex items-center justify-between">
                ...
                <button onClick={() => setShowMembers(!showMembers)} className="text-teal-600">
                    <Users size={22} />
                </button>
            </div>
                    ...
                    <div className="flex gap-3 overflow-x-auto">
                        {members.map((member) => (
                            <div key={member.id} className="flex flex-col items-center min-w-fit">
                                <img src={getMediaUrl(member.profile_pic)} alt={member.username}
                ...
                {messages.map((msg) => {
                    ...
                    return (
                        <div key={msg.id} className={`flex gap-2 ${isMine ? "justify-end" : "justify-start"}`}>
                            {!isMine && (
                                <img src={getMediaUrl(msg.sender_profile_pic)} alt="sender"
                                    className="w-8 h-8 rounded-full object-cover self-end" />
                            )}
                            <div className={`max-w-xs ${isMine ? "items-end" : "items-start"} flex flex-col`}>
                                {!isMine && <p className="text-xs text-gray-400 mb-1">@{msg.sender_username}</p>}
                                <div className={`px-4 py-2 rounded-2xl text-sm ${isMine ? "bg-teal-500 text-white rounded-br-none" : "bg-white text-gray-800 shadow rounded-bl-none"}`}>
                                    {msg.content}
                                </div>
