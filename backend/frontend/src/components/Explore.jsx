        ...
        <div className="h-screen flex flex-col pb-16">
                ...
                <div className="flex gap-2 overflow-x-auto pb-1">
                    {filterTabs.map(tab => (
                        <button key={tab.key} onClick={() => setFilter(tab.key)}
                            className={`px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap text-white ${filter === tab.key ? tab.color : 'bg-gray-300'}`}>
                            {tab.label}
                        </button>
                    ))}
                    {filter === 'joinable' && (
                        <div className="flex items-center gap-2 ml-2">
                            <span className="text-xs text-gray-500 whitespace-nowrap">Budget: ₹{budgetMax.toLocaleString()}</span>
                            <input type="range" min={1000} max={200000} step={1000} value={budgetMax}
                                onChange={(e) => setBudgetMax(Number(e.target.value))} className="w-24" />
                        </div>
                    )}
                </div>
                <div className="flex gap-3 mt-2">
                    <span className="flex items-center gap-1 text-xs text-gray-500"><span className="w-3 h-3 rounded-full bg-blue-500 inline-block"></span> Trips</span>
                    <span className="flex items-center gap-1 text-xs text-gray-500"><span className="w-3 h-3 rounded-full bg-teal-500 inline-block"></span> Experiences</span>
                    <span className="flex items-center gap-1 text-xs text-gray-500"><span className="w-3 h-3 rounded-full bg-purple-500 inline-block"></span> Posts</span>
                </div>
            </div>
                    ...
                    <>
                        <MapContainer center={[20.5937, 78.9629]} zoom={5} style={{ height: "100%", width: "100%" }}>
                            ...
                            {joinableWithCoords.map((trip) => (
                                <Marker key={`joinable-${trip.id}`} position={[trip.latitude, trip.longitude]} icon={joinableIcon}>
                                    <Popup>
                                        <div className="w-48">
                                            {getImage(trip) && <img src={getImage(trip)} alt={trip.title} className="w-full h-24 object-cover rounded-lg mb-2" />}
                                            ...
                                            <button onClick={() => navigate(`/joinable-trip/${trip.id}`)}
                                                className="mt-2 w-full bg-blue-500 text-white text-xs py-1 rounded-full font-semibold">View Trip</button>
                                        </div>
                                    </Popup>
                                </Marker>
                            ))}
                            ...
                            {experienceWithCoords.map((trip) => (
                                <Marker key={`exp-${trip.id}`} position={[trip.latitude, trip.longitude]} icon={experienceIcon}>
                                    <Popup>
                                        <div className="w-48">
                                            {getImage(trip) && <img src={getImage(trip)} alt={trip.title} className="w-full h-24 object-cover rounded-lg mb-2" />}
                                            <p className="font-bold text-sm">{trip.title}</p>
                                            ...
                                            <button onClick={() => navigate(`/trip/${trip.id}`)}
                                                className="mt-2 w-full bg-teal-500 text-white text-xs py-1 rounded-full font-semibold">View Experience</button>
                                        </div>
                                    </Popup>
                                </Marker>
                            ))}
                            ...
                            {generalWithCoords.map((post) => (
                                <Marker key={`general-${post.id}`} position={[post.latitude, post.longitude]} icon={generalIcon}>
                                    <Popup>
                                        <div className="w-48">
                                            {post.images?.[0]?.image && <img src={getMediaUrl(post.images[0].image)} alt="post" className="w-full h-24 object-cover rounded-lg mb-2" />}
                                            ...
                                            <button onClick={() => navigate(`/general-post/${post.id}`)}
                                                className="mt-2 w-full bg-purple-500 text-white text-xs py-1 rounded-full font-semibold">View Post</button>
                                        </div>
                                    </Popup>
                                </Marker>
                            ))}
            ...
            {filter === 'joinable' && joinableTrips.length > 0 && (
                <div className="bg-white border-t">
                    <p className="px-4 pt-2 text-xs font-semibold text-gray-400">{joinableTrips.length} trips · {joinableWithCoords.length} on map</p>
                    <div className="flex gap-3 px-4 py-2 overflow-x-auto">
                        {joinableTrips.slice(0, 10).map((trip) => (
                            <div key={trip.id} onClick={() => navigate(`/joinable-trip/${trip.id}`)}
                                ...
                                <div className="p-2">
                                    ...
                                    {trip.latitude && <p className="text-xs text-green-500">📌 On map</p>}
                                </div>
                            </div>
                        ))}
                    </div>
