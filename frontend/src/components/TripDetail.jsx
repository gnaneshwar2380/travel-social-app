...
export default function TripDetail() {
    ...
    useEffect(() => {
        const fetchTripData = async () => {
            try {
                const [resPost, userRes] = await Promise.all([
                    api.get(`/posts/${id}/`),
                    api.get("/profile/"),
                ]);
    ...
    const handleDelete = async () => {
        ...
        try {
            await api.delete(`/posts/${id}/edit/`);
            navigate("/");
        } catch (err) { console.error(err); }
    };
    ...
    return (
        <div className="max-w-3xl mx-auto px-4 py-6 pb-24">
                                ...
                                {showMenu && (
                                    <>
                                        ...
                                        <div className="absolute right-0 top-8 bg-white rounded-xl shadow-lg border z-50 w-36 overflow-hidden">
                                            <button onClick={() => { navigate(`/trip/${id}/edit`); setShowMenu(false); }}
            ...
            <div className="space-y-8 mb-8">
                {days.length === 0 && <p className="text-center text-gray-400">No days added yet.</p>}
                {days.map((day) => (
                    <div key={day.id} className="bg-white rounded-xl shadow overflow-hidden">
                        <div className="p-5 border-b">
                            ...
                            {day.date && <p className="text-sm text-gray-400 mt-1">{new Date(day.date).toLocaleDateString()}</p>}
